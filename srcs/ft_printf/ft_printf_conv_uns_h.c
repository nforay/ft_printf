...
static int		ft_conv_base(t_state_machine *m, int len, unsigned short int nbr, char *base)
{
...
static void	ft_putnbr_base(t_state_machine *m, unsigned short int nbr, char *base)
{
	if (nbr / 16 != 0)
...
{
	if (m->flag & (HASH) && m->args.hux && !(m->flag & MINUS) && m->flag & POINT)
	{
...
void	print_conv_uns_h(t_state_machine *m)
{
	...
	if (!(m->args.hux) && m->preci == 0 && m->flag & POINT && m->fwidth == 0)
		return ;
	strlen = ft_conv_base(m, 0, m->args.hux, (m->flag & U_CONV) ? B_TEN : B_HEX);
	print_hash(m, strlen);
	...
	if (!(m->args.hux) && m->preci == 0 && m->flag & POINT)
	{
		...
		ft_putnbr_base(m, m->args.hux, B_HEX);
	else if (m->flag & XMAJ_CONV)
		ft_putnbr_base(m, m->args.hux, B_MHEX);
